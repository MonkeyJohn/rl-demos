...
class PGAgent:
	...
	def learn(self, render=False, max_episodes=1000, batch_size=10, 
		...
		dataQ = tf.FIFOQueue(10000,[tf.float32,tf.int32,tf.float32], shapes=[(1,self.inp_dim),(),()])
		#xs,ys,rs=[],[],[]
		prev_x = None
		while episode_number <= max_episodes:
			...
			#ys.append(action)
			#rs.append(reward)
			data_enqueue_op = dataQ.enqueue([x,action,reward])
			self.sess.run(data_enqueue_op)

			if done:
				episode_number += 1
				xs, ys, rs = self.sess.run(dataQ.dequeue())
