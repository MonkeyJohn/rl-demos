...
def runEpisode(agent, environment, discount, decision, display, message, pause, episode):
    ...
    while True:
        ...
        if len(actions) == 0:
            ...
            #for SARSA
            if 'sampleAction' in dir(agent):
                agent.sampleAction = None
...
if __name__ == '__main__':
    ...
    import valueIterationAgents, qlearningAgent, sarsaLearningAgent
    a = None
    if opts.agent == 'value':
        ...
        gridWorldEnv = GridworldEnvironment(mdp)
        actionFn = lambda state: mdp.getPossibleActions(state)
        sLearnOpts = {'gamma': opts.discount,
                      'alpha': opts.learningRate,
                      'epsilon': opts.epsilon,
                      'actionFn': actionFn}
        a = sarsaLearningAgent.SarsaLearningAgent(**sLearnOpts)        
